...
function cleanup(settings, callback) {
  ...
  function purgeKey(key, callback) {
    if (settings.emulate) {
      console.log('[EMULATION] would purge entry with key '+key);
      return callback();
    }
    console.log('going to purge key ' + key);
  ...
  function deleteKey(key, callback) {
    async.waterfall([function(callback) {
      if (settings.purge) {
        purgeKey(key, callback);
      }
      return callback();
    }, function(callback) {
      if ((!settings.contentPath || !regexContent) && !settings.purgeZombies) {
        return callback(null, true);
      }
      db.get(settings.bucket, key, function(err, data) {
        if (err) {
          console.log('failed to load key: %j', key);
          if (err.notFound && settings.purgeZombies) {
            purgeKey(key, function() {
              return callback(null, true);
            });
            return;
          }
        ...
        if (settings.purgeZombies) {
          console.log('entry %j found will not be deleted', key);
